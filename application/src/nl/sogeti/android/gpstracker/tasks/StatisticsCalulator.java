 ...
 *
 */
package nl.sogeti.android.gpstracker.tasks;
...
public class StatisticsCalulator extends AsyncTask<Uri, Void, Void>
{
   ...
   private void updateCalculations(Uri trackUri)
   {
      ...
      try
      {
         waypointsCursor = resolver.query(Uri.withAppendedPath(trackUri, "waypoints"), new String[] { "max  (" + Waypoints.TABLE + "." + Waypoints.SPEED + ")",
         ...
         waypointsCursor = resolver.query(Uri.withAppendedPath(trackUri, "waypoints"), new String[] { "avg  (" + Waypoints.TABLE + "." + Waypoints.SPEED + ")" }, Waypoints.TABLE + "."
               + Waypoints.SPEED + "  > ?", new String[] { "" + Constants.MIN_STATISTICS_SPEED }, null);
         if (waypointsCursor.moveToLast())
         {
